...
pygame.display.set_caption('Space Invaders by- kshitij')
...
def main():
    ...
    lost_font= pygame.font.SysFont('algerian',50)
    ...
    clock = pygame.time.Clock() #starting clock to check for collisions and all after every FPS
    
    def redraw_window():
        WIN.blit(BG,(0,0)) #draws the background on the surface to the given coordinates
    ...
    while run:
        ...
        if len(enemies)==0:
            ...
            for i in range(wave_length):
                enemy=Enemy(random.randrange(50 , WIDTH-100), random.randrange(-1500, -100), random.choice(['red','blue','green']))#enemy doesn't go out of range
                enemies.append(enemy)
      
        for event in pygame.event.get(): #check for all events in pygame and if an event occurs, do something
            if event.type == pygame.QUIT:
              
                quit()
                
        keys = pygame.key.get_pressed() #get dictionary of all keys weather pressed or not
